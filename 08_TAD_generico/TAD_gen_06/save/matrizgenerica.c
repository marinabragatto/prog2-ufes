...
tMatrizGenerica *MultiplicaMatrizes(tMatrizGenerica *mat1, tMatrizGenerica *mat2, int numByteTarget, void* (*multi_elem)(void*, void*), void* (*soma_elem)(void*,void*)){
    ...
    for(i = 0; i < res->lin; i++){
        for(j = 0; j < res->col; j++){
            ...
            //multi_elem(res->data[i][j], 0);
            
            for(m = 0; m <  ord; m++){
                mult = multi_elem(mat1->data[i][m], mat2->data[m][j]);
                AtribuiElementoMatrizGenerica(res,i,j,soma_elem(res->data[i][j], mult));
...
tMatrizGenerica *ConverteTipoMatriz(tMatrizGenerica *mat2, int novoNumByteElem, void* (*converte_elem)(void*)){
    ...
    //mat3 = mat2;
    //mat3 = CriaMatrizGenerica(mat2->lin, mat2->col, mat2->novoNumByteElem);
    //mat3 =  converte_elem(mat2);
    int j = 0, i = 0;
    for(i = 0; i < mat2->lin; i++){
        for(j = 0 ; j < mat2->col; j++){
            //free(mat2->data[i][j]);
            //mat2->data[i][j] = (void*)malloc(novoNumByteElem);
            AtribuiElementoMatrizGenerica(mat2,i,j, converte_elem(mat2->data[i][j]));
            //mat2->data[i][j] = 
        }
    }
