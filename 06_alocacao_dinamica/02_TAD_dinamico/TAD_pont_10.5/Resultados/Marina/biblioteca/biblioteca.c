...
int ehMultiploDeCinco(int numero);
...
tBiblioteca* InicializarBiblioteca(){
    ...
    int i = 0;
    biblioteca->livros = (tLivros**)malloc(sizeof(tLivros*)*NUM_INICIAL_LIVROS);
...
void AdicionarLivroNaBiblioteca(tBiblioteca* biblioteca, tLivros* livro){
    if(ehMultiploDeCinco(biblioteca->tamanho) && biblioteca->tamanho != 0){
        biblioteca->tam_max_atual += 5;
        biblioteca->livros = (tLivros**)realloc(biblioteca->livros, sizeof(tLivros*)*(biblioteca->tam_max_atual));
