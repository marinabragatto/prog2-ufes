...
tPonto Pto_Cria (float x, float y){
    tPonto pnt;
    pnt = (tPonto)malloc(sizeof(struct ponto));
    pnt->x = x;
    pnt->y = y;
    return pnt;
}
...
float Pto_Distancia (tPonto p1, tPonto p2){
    float dist = 0;
    dist = (Pto_Acessa_x(p1) - Pto_Acessa_x(p2))*(Pto_Acessa_x(p1) - Pto_Acessa_x(p2)) +(Pto_Acessa_y(p1) - Pto_Acessa_y(p2))*(Pto_Acessa_y(p1) - Pto_Acessa_y(p2)) ;
    dist = sqrt (dist);
    return dist;
}